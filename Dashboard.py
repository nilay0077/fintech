...
def load_data(start_date, end_date):
    # Load the ETF data
    try:
        etf_df = pd.read_csv("all_etf_data (1).csv")  # Updated to relative path
    ...
    try:
        sp500_df = pd.read_csv("S&P 500 Historical Data.csv")  # Updated to relative path
...
with tab1:
    ...
    fig_bubble.update_layout(
        ...
        yaxis_title="Alpha (%)",
        xaxis=dict(range=[0, 21], gridcolor='white', zeroline=False, tickvals=[0, 2.6, 5, 7.6, 10, 12.6, 15, 17.6, 20]),
        yaxis=dict(range=[0, 8], gridcolor='white', zeroline=False, tickvals=[0, 1, 2, 3, 4, 5, 6, 7, 8]),
        plot_bgcolor='black',
        paper_bgcolor='black',
...
with tab2:
    ...
    try:
        pipeline_img = Image.open("DSPipiline.png")  # Updated to relative path
