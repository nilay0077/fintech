...
with tab1:
    ...
    for method, marker in zip(['LSTM', 'Transformer', 'RNN'], ['circle', 'square', 'triangle-up']):
        ...
        fig_bubble.add_trace(
            go.Scatter(
                ...
                textfont=dict(size=12, color='black')  # Changed to black for visibility on white background
            )
        )
    ...
    fig_bubble.update_layout(
        ...
        font=dict(color='black', size=14),  # Changed to black for visibility
        title_font=dict(size=16, color='black'),  # Changed to black
        legend=dict(
            title="Model",
            font=dict(color='black'),  # Changed to black
            orientation="v",
            yanchor="top",
            y=1,
            xanchor="left",
