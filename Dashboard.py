...
def load_data(start_date, end_date):
    # Load the ETF data
    try:
        etf_df = pd.read_csv("all_etf_data (1).csv")  # Relative path
        etf_df['Date'] = pd.to_datetime(etf_df['Date'], format='%d-%m-%Y', errors='coerce')
    except FileNotFoundError:
        raise FileNotFoundError("ETF data file not found at 'all_etf_data (1).csv'. Please ensure the file is in the same directory as the app.")
    ...
    try:
        sp500_df = pd.read_csv("S&P 500 Historical Data.csv")  # Relative path
        sp500_df['Date'] = pd.to_datetime(sp500_df['Date'], format='%m/%d/%Y', errors='coerce')
    except FileNotFoundError:
        raise FileNotFoundError("S&P 500 data file not found at 'S&P 500 Historical Data.csv'. Please ensure the file is in the same directory as the app.")
...
with tab1:
    ...
    portfolios = ['Model 1 (Sentiment Only)', 'Model 2 (Sentiment + Volatility)', 'Model 3 (Sentiment + Macro)']
    ...
    method_comparison_data = pd.DataFrame({
        'Model': ['Model 1 (Sentiment Only)', 'Model 1 (Sentiment Only)', 'Model 1 (Sentiment Only)',
                  'Model 2 (Sentiment + Volatility)', 'Model 2 (Sentiment + Volatility)', 'Model 2 (Sentiment + Volatility)',
                  'Model 3 (Sentiment + Macro)', 'Model 3 (Sentiment + Macro)', 'Model 3 (Sentiment + Macro)'],
    ...
    method_comparison_data['Label'] = method_comparison_data['Model'].str.replace('Model ', 'Mod').str.replace(r' \([^)]+\)', '', regex=True)
    ...
    # Update layout to match the provided chart, with white background
    fig_bubble.update_layout(
        title="Bubble Chart: Alpha vs Volatility (Bubble Size = Return)",
        xaxis_title="Volatility (%)",
        yaxis_title="Alpha (%)",
        xaxis=dict(range=[0, 21], gridcolor='lightgray', zeroline=False, tickvals=[0, 2.6, 5, 7.6, 10, 12.6, 15, 17.6, 20]),
        yaxis=dict(range=[0, 8], gridcolor='lightgray', zeroline=False, tickvals=[0, 1, 2, 3, 4, 5, 6, 7, 8]),
        plot_bgcolor='white',
        paper_bgcolor='white',
    ...
    weight_data = pd.DataFrame({
        'Model': ['Model 1 (Sentiment Only)', 'Model 1 (Sentiment Only)', 'Model 1 (Sentiment Only)', 'Model 1 (Sentiment Only)', 'Model 1 (Sentiment Only)',
                  'Model 2 (Sentiment + Volatility)', 'Model 2 (Sentiment + Volatility)', 'Model 2 (Sentiment + Volatility)', 'Model 2 (Sentiment + Volatility)', 'Model 2 (Sentiment + Volatility)',
                  'Model 3 (Sentiment + Macro)', 'Model 3 (Sentiment + Macro)', 'Model 3 (Sentiment + Macro)', 'Model 3 (Sentiment + Macro)', 'Model 3 (Sentiment + Macro)'],
...
with tab2:
    ...
    try:
        pipeline_img = Image.open("DSPipiline.png")  # Relative path
        st.image(pipeline_img, caption="Data Foundation Process Flow", output_format="PNG")
    except FileNotFoundError:
        st.error("Pipeline image not found at 'DSPipiline.png'. Please ensure the file is in the same directory as the app.")

    st.subheader("Distribution of Unique News Articles Across Different Sectors")
    sector_data = pd.DataFrame({
        "Sector": sectors,
        "Percentage": [16.78, 22.12, 24.08, 7.31, 29.72]
    })
